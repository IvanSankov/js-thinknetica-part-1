// Для системы контроля авиабилетов, рассмотренной на занятии.

// Point a Добавить функцию возврата билета
...
function revertTicket(ticket, nowTime) {
    ...
    if (ticketObj.index === null) {
        return true;
    }

    if (ticketObj.ticket.type === 1) {
        return false;
    }

    flight.tickets.splice(ticketObj.index, 1);
    flight.countOfReverts++;
    return true;
}

/**
 * Функция возвращает название рейста по номеру билета
 *
 * *
 *
 * @param {string} ticket
 * @return {string}
 */
function getFlightNameByTicket(ticket) {
    try {
...
/**
 * Функция возвращает рейс по названию
 *
 * @param {string} name
...
/**
 * Функция возвращает количество миллесекунд в hours
 *
 * @param {number} hours
 * @return {number}
 */
function getHoursTimestamp(hours) {
 ...
 * @return {Object} Flight
 */
function getTicketById(flight, ticket) {
    return flight.tickets.reduce((acc, currVal, currIndex) => {
        if (currVal.id === ticket) {
            acc.index = currIndex;
            acc.ticket = currVal;
        }
...
// Point b Добавить в генератор отчета
...
function flightReport(flight, nowTime) {
    let flightObj = getFlightByName(flight);
    ...
    return {
        ...
        countOfReverts: flightObj.countOfReverts,
        percentOfReverts: flightObj.countOfReverts !== 0 ? flightObj.countOfReservations / flightObj.countOfReverts : 0,
    };
}
